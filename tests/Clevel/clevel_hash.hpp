...
public:
		     ...
		     size_type capacity) const
	{
		...
		// 0xc6a4a7935bd1e995 is the hash constant from 64-bit
		// MurmurHash2
		uint64_t hash_of_tag =
			(uint64_t)(nonzero_tag * 0xc6a4a7935bd1e995);
	...
	difference_type
	alt_index(partial_t partial, difference_type idx,
		  size_type capacity) const
	{
		partial_t nonzero_tag = (partial >> 1 << 1) + 1;
		// 0xc6a4a7935bd1e995 is the hash constant from 64-bit
	...
	struct ret {
		...
		ret(size_type _level_idx, difference_type _bucket_idx,
		    size_type _slot_idx, bool _expanded = false,
		    uint64_t _cap = 0)
		    : found(true),
	...
	clevel_hash()
	    : meta(make_persistent<level_meta>().raw().off), thread_num(0)
	{
		...
		persistent_ptr<level_bucket> tmp =
			make_persistent<level_bucket>();
		...
		KV_entry_ptr_t e =
			get_entry(meta(my_pool_uuid)->first_level, 0, 0);
		if (e != nullptr) {
			// never fires.
			get_key(e);
		}
	...
	capacity(level_meta_ptr_t m_copy) const
	{
		...
		total_slots +=
			li.get_address(my_pool_uuid)->capacity * assoc_num;
	...
	{
		if (thread_num > 0) {
			// Reclaim the memory in persistent buffers allocated in
			// previous round of set_thread_num.
			for (size_type i = 0; i < thread_num; i++) {
				difference_type di =
					static_cast<difference_type>(i);
				if (tmp_entry[di] != nullptr)
					delete_persistent<value_type>(
						tmp_entry[di]);
			}
		...
		tmp_entry = make_persistent<persistent_ptr<value_type>[]>(
			thread_num);
	}
	...
	f_code_t find(pool_base &pop, const key_type &key, partial_t partial,
		      size_type &n_levels, KV_entry_ptr_t &old_e,
	...
	void expand(pool_base &pop, size_type thread_id,
		    level_meta_ptr_t m_copy);
...
template <typename Key, typename T, typename Hash, typename KeyEqual,
	  size_t HashPower>
void
clevel_hash<Key, T, Hash, KeyEqual, HashPower>::del_dup(pool_base &pop,
							KV_entry_ptr_u *p1,
							KV_entry_ptr_u *p2,
							KV_entry_ptr_t e1,
							KV_entry_ptr_t e2)
...
clevel_hash<Key, T, Hash, KeyEqual, HashPower>::find_empty_slot(
	...
	size_type &n_levels, KV_entry_ptr_t **e, uint64_t &level_num,
	level_meta_ptr_t &m_copy)
{
	hv_type hv = hasher{}(key);
	while (true) {
		...
		for (size_type i = n_levels - 1; i < n_levels; i--) {
			...
			for (size_type j = 0; j < assoc_num; j++) {
				if (!found_empty_in_b &&
					...
					if (result == VACANCY_IN_LEFT &&
					    level_num == i && slot_idx <= j)
						continue;
		...
		*e = nullptr;
		...
		for (size_type i = 0; i < n_levels; i++) {
			...
			for (size_type j = 0; j < assoc_num; j++) {
				f_e = f_b.slots[j].p;
				if (f_e.get_offset() == 0) {
					// Since empty slots in top levels are
					// preferred, update vacancy info as
					// long as identical keys are not found.
					if (result != FOUND_IN_LEFT &&
				...
				if (f_b.slots[j].x.partial != partial ||
				    !key_equal{}(f_e.get_address(my_pool_uuid)
							 ->first,
						 key))
					continue;
				...
				if (result == FOUND_IN_LEFT ||
					...
					if (f_e.get_offset() ==
						...
						// re-insertion in normal
						// executions or os scheduling
						// during a rehashing operation.
						// In this case, delete the
						// pointer in bottom level.
						if (prev_i < i) {
							del_dup(pop,
								&f_b.slots[j],
								&(levels[level_num]
					...
					// Refer to different locations
						...
						// Duplication due to the
						// re-insertion after a crash or
						// concurrent insertions of same
						// key. To fix the duplication,
						// simply delete the previous
						// item.
					...
					*e = &(f_b.slots[j].p);
					...
					prev_e = f_e;
					prev_i = i;
				} // end if result in FOUND_IN_LEFT or
				  // FOUND_IN_RIGHT
			}	  // end for j, f_idx, f_b
			...
			for (size_type j = 0; j < assoc_num; j++) {
				...
				if (s_b.slots[j].x.partial != partial ||
				    !key_equal{}(s_e.get_address(my_pool_uuid)
							 ->first,
						 key))
					continue;
	...
	size_type thread_id, size_type id)
{
	...
	persistent_ptr<value_type> tmp_e;
	allocate_KV(pop, tmp_e, param);
	KV_entry_ptr_u created(tmp_e.raw().off);
	...
	RETRY_INSERT:
		...
		f_code_t result;
		if (check_duplicate) {
			result = find(pop, key, partial, n_levels, old_e, &e,
				      level_num, idx, /*fix_dup=*/false,
				      thread_id, m_copy);
		} else {
		...
		if (result == FOUND_IN_LEFT || result == FOUND_IN_RIGHT) {
			...
			delete_persistent_atomic<value_type>(tmp_e);
			return ret(level_num, 0, 0);
		} else if ((result == VACANCY_IN_LEFT ||
			    result == VACANCY_IN_RIGHT) &&
			   (level_num > 0 || !m->is_resizing)) {
			if (CAS(&(e->off), old_e.raw(), created.p.raw())) {
				if (!m->is_resizing &&
				    meta(my_pool_uuid)->is_resizing &&
				    level_num == 0) {
					// Resizing may occur during the insert.
					// Hence, redo the insertion to avoid
					// missing the new item. The possible
					// duplication will be fixed in future
					// updates and deletes.
					pop.persist(&(meta.off),
						    sizeof(uint64_t));
					...
					return ret(expanded_flag,
						   initial_capacity);
				}

			} else {
#ifdef CLEVEL_DEBUG
				std::cout << "insertion, cas fails, n_levels: "
					  << n_levels << std::endl;
#endif
				goto RETRY_INSERT;
			}
	...
	size_type thread_id)
{
	...
	while (true) {
		...
		f_code_t result =
			find(pop, key, partial, n_levels, old_e, &e, level_num,
			     idx, /*fix_dup=*/true, thread_id, m_copy);

		if (result == FOUND_IN_LEFT || result == FOUND_IN_RIGHT) {
			if (succ_update && old_e == created.p) {
				// The only item in table after update is the
				// modified one, which indicates a successful
				// update.
				return ret(true);
			} else if (CAS(&(e->off), old_e.raw(),
				       created.p.raw())) {
				pop.persist(&(e->off), sizeof(uint64_t));

				// Instead of simply issuing another find to
				// guarantee the update is successful, we apply
				// context checking to avoid unnecessary second
				// find. The update fails only when the item to
				// be updated is copied by rehashing threads
				// after find and before update's CAS.
				// Therefore, we can do context checking to
				// avoid such failure.
				if (m_copy != meta ||
				    (level_num == 0 && idx <= expand_bucket &&
			...
			if (!succ_update) {
				// delete_persistent_atomic<value_type>(
				// 	tmp_entry[t_id]);
				delete_persistent_atomic<value_type>(tmp_e);
			}
			// Even the updated item is deleted by other threads,
			// our update succeeds anyway.
...
template <typename Key, typename T, typename Hash, typename KeyEqual,
	  size_t HashPower>
void
clevel_hash<Key, T, Hash, KeyEqual, HashPower>::expand(pool_base &pop,
						       size_type thread_id,
						       level_meta_ptr_t m_copy)
{
	level_meta *m = static_cast<level_meta *>(m_copy(my_pool_uuid));
	...
	if (cl->up == nullptr) {
		persistent_ptr<level_bucket> tmp_level;
		make_persistent_atomic<level_bucket>(pop, tmp_level);
		size_type new_capacity = cl->capacity * 2;
		std::cout << "Thread-" << thread_id << " starts expanding for "
			  << new_capacity << " buckets" << std::endl;

		make_persistent_atomic<bucket[]>(pop, tmp_level->buckets,
						 new_capacity);

		pop.persist(tmp_level->buckets);
		tmp_level->capacity = new_capacity;
		pop.persist(tmp_level->capacity);
		tmp_level->up = nullptr;
		pop.persist(&(tmp_level->up.off), sizeof(uint64_t));

		// Append a new level.
		bool rc = CAS(&(cl->up.off), 0, tmp_level.raw().off);

		if (rc == false) {
			// Ohter threads finished expanding
			pop.persist(&(cl->up.off), sizeof(uint64_t));

			delete_persistent_atomic<bucket[]>(tmp_level->buckets,
							   new_capacity);

			delete_persistent_atomic<level_bucket>(tmp_level);
		}
		...
		while (true) {
			persistent_ptr<level_meta> tmp_meta;
			if (cl->capacity >= new_capacity) {
				// Help updating meta
				make_persistent_atomic<level_meta>(
					pop, tmp_meta, m->first_level,
					m->last_level, true);
			} else {
				assert(cl->up != nullptr);
				make_persistent_atomic<level_meta>(
					pop, tmp_meta, cl->up, m->last_level,
					true);
			}

			if (CAS(&(meta.off), m_copy.off, tmp_meta.raw().off)) {
				pop.persist(&(meta.off), sizeof(uint64_t));
				...
				if (cl->capacity >= new_capacity &&
					...
					// updating meta
					delete_persistent_atomic<level_meta>(
						tmp_meta);
					break;
				}
		...
		if (meta == m_copy) {
			...
			// Update the first_level and is_resizing in the
			// metadata.
			while (true) {
				// Help updating meta
				persistent_ptr<level_meta> tmp_meta;
				...
				if (CAS(&(meta.off), m_copy.off,
					...
					m = static_cast<level_meta *>(
						m_copy(my_pool_uuid));
					cl = m->first_level.get_address(
						my_pool_uuid);

					if (cl->capacity >= new_capacity &&
					    m->is_resizing) {
						// CAS fails because other
						// threads help updating meta
						delete_persistent_atomic<
							level_meta>(tmp_meta);
						break;
					}
					// CAS fails because other threads
					// complete rehashing.
				}
			}
		}
...
clevel_hash<Key, T, Hash, KeyEqual, HashPower>::resize()
{
	...
	while (run_expand_thread.get_ro().load()) {
		...
		RETRY_REHASH:
			...
			level_bucket *bl =
				m->last_level.get_address(my_pool_uuid);
			level_bucket *tl =
				m->first_level.get_address(my_pool_uuid);

			bucket &b = bl->buckets[expand_bucket.get_ro()];
			for (size_type slot_idx = 0; slot_idx < assoc_num;
			     slot_idx++) {
				KV_entry_ptr_t src_tmp = b.slots[slot_idx].p;
				value_type *e =
					src_tmp.get_address(my_pool_uuid);
				if (e == nullptr)
					continue;
				...
				for (size_type j = 0; j < assoc_num; j++) {
					// The rehashed item is inserted into
					// the less-loaded bucket between the
					// two candidata buckets in the new
					// level.
					KV_entry_ptr_t dst_tmp =
						dst_b1.slots[j].p;
					if (dst_tmp.get_offset() == 0) {
						if (CAS(&(dst_b1.slots[j]
								  .p.off),
							dst_tmp.raw(),
							src_tmp.raw())) {
							pop.persist(
				...
				if (!succ) {
					std::cout << "expand during resizing!"
						  << std::endl;
					expand(pop, thread_id, m_copy);
					goto RETRY_REHASH;
				}
			...
			if (static_cast<size_type>(expand_bucket) ==
			    bl->capacity) {
				bool rc = false;
				while (true) {
					...
					if (CAS(&(meta.off), m_copy.off,
						tmp_meta.raw().off)) {
						std::cout
							<< "Expand thread updates metadata, "
							<< "is_resizing: "
							<< bool(levels_left !=
								2)
							<< " levels_left: "
