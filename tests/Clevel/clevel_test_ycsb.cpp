...
#ifdef MACRO_TEST
// (2^18 + 2^17) * 8 = 3145728
#define HASH_POWER 18
#define OPERATION_NUM 64000000
#else
// (2^17 + 2^16) * 8 = 1572864
#define HASH_POWER 17
...
using persistent_map_type =
	nvobj::experimental::clevel_hash<string_t, string_t, string_hasher,
...
main(int argc, char *argv[])
{
	...
	if (file_exists(path)) {
		...
		nvobj::transaction::run(pop, [&] {
			...
			pop.root()->cons->set_thread_num(2);
		});
	}
	...
	std::ofstream ofs_loadfactor("clevel_loadfactor.res");
	...
	while (ifs_load >> opstr) {
		...
		if (op == OP::PUT) {
			...
			auto ret = map->insert(
				persistent_map_type::value_type(key, key), 1,
				loaded);
			if (!ret.found) {
				loaded++;
#ifdef LOADFACTOR_TEST
	...
	std::ofstream ofs_throughput("clevel_throughput.res");
...
#ifdef LATENCY_ENABLE
	std::ofstream ofs_latency("clevel_latency.res");
