...
public:
	...
	using clht_hashtable_ptr_t =
		detail::compound_pool_ptr<clht_hashtable_s>;
	...
	struct ret {
		...
		ret(difference_type _bucket_idx, uint8_t _step,
		    uint8_t _slot_idx, size_type _expanded = false,
		    uint64_t _cap = 0)
		    : found(true),
	...
	struct ALIGNED(CACHE_LINE_SIZE) clht_hashtable_s {
		...
		clht_hashtable_s(uint64_t n_buckets = 0)
		    : num_buckets(n_buckets)
		{
			...
			table_prev = nullptr;
			num_expands = 0;
			num_expands_threshold =
				(CLHT_PERC_EXPANSIONS * num_buckets);
		...
		do {
			...
			if (likely(bucket->next == nullptr)) {
				if (unlikely(empty == nullptr)) {
					bucket_ptr_t b_new =
						clht_bucket_create_stats(
							pop, ht_ptr, resize);
					kv_ptr_t &s_new =
						b_new(my_pool_uuid)->slots[0];

					s_new.off = tmp_entry.raw().off;
					pop.persist(&s_new.off,
						    sizeof(kv_ptr_t));
					bucket->next = b_new;
					pop.persist(&bucket->next.off,
						    sizeof(bucket_ptr_t));
				} else {
					empty->off = tmp_entry.raw().off;
					pop.persist(&empty->off,
						    sizeof(kv_ptr_t));
				}
				...
				if (unlikely(resize)) {
					...
					expanded = true;
#endif
					// Start resizing... If crash, return
					// true, because the insert anyway
					// succeeded
				...
				if (bucket->slots[j] != nullptr &&
				    key_equal{}(
					    bucket->slots[j]
						    ...
						    ->first,
					    key)) {
					PMEMoid oid = bucket->slots[j].raw_ptr(
						my_pool_uuid);
					pmemobj_free(&oid);

					bucket->slots[j] = nullptr;
					pop.persist(&bucket->slots[j].off,
						    sizeof(kv_ptr_t));
	...
	size()
	{
		...
		for (difference_type idx = 0; idx < n_buckets; idx++) {
			...
			do {
				for (size_t j = 0; j < ENTRIES_PER_BUCKET;
				     j++) {
					if (bucket->slots[j] != nullptr)
						size++;
				}
	...
	ht_status(bool is_increase, bool just_print)
	{
		...
		clht_hashtable_s *ht_ptr = ht.get_address(my_pool_uuid);
		difference_type n_buckets =
			(difference_type)ht_ptr->num_buckets;
		bucket_s *bucket = nullptr;
		size_type size = 0;
		int expands = 0;
		int expands_max = 0;
		...
		double full_ratio =
			100.0 * size / (n_buckets * ENTRIES_PER_BUCKET);

		if (just_print) {
			printf("[STATUS-%02d] #bu: %7zu / #elems: %7zu / full%%: %8.4f%% / expands: %4d / max expands: %2d\n",
			       99, n_buckets, size, full_ratio, expands,
			       expands_max);
		} else {
			if (full_ratio > 0 &&
			    full_ratio < CLHT_PERC_FULL_HALVE) {
				ht_resize_pes(false /* is_increase */,
					      (size_t)33);
			} else if ((full_ratio > 0 &&
				    full_ratio > CLHT_PERC_FULL_DOUBLE) ||
				   expands_max > CLHT_MAX_EXPANSIONS ||
				   is_increase) {
				uint64_t inc_by = (uint64_t)(
					full_ratio / CLHT_OCCUP_AFTER_RES);
	...
	ht_resize_pes(bool is_increase, size_t by)
	{
		...
		if (is_increase) {
			/* num_buckets_new = CLHT_RATIO_DOUBLE *
			 * ht_old->num_buckets; */
...
#endif
			num_buckets_new =
				ht_old->num_buckets / CLHT_RATIO_HALVE;
		}

		persistent_ptr<clht_hashtable_s> ht_tmp;
		transaction::run(pop, [&] {
			// A transaction is required as the constructor of
			// clht_hashtable_s invokes "make_persistent".
			ht_tmp = make_persistent<clht_hashtable_s>(
				num_buckets_new);
		});
		...
		pop.persist(&ht_old->table_tmp.off,
			    sizeof(clht_hashtable_ptr_t));
		...
		if (is_increase &&
		    ht_old->is_helper != 1) /* there exist a helper */
		{
		...
		pop.persist(&ht_old->table_new.off,
			    sizeof(clht_hashtable_ptr_t));

		unlock(pop, &resize_lock);

		if (ht_resize_again)
			ht_status(true /* is_increase */,
				  false /* just_print */);
					...
					hv_type hv = hasher{}(
						bucket->slots[j]
							.get_address(
								my_pool_uuid)
							->first);
					difference_type idx = static_cast<
						difference_type>(
						hv %
						static_cast<hv_type>(
							ht_new->num_buckets));
	...
	ht_resize_help(clht_hashtable_s *hashtable)
	{
		...
		for (difference_type idx = (difference_type)hashtable->hash;
			...
			if (!bucket_cpy(bucket,
					hashtable->table_tmp.get_address(
						my_pool_uuid)))
				break;
		}
	...
	print()
	{
		...
		difference_type n_buckets =
			(difference_type)hashtable->num_buckets;
