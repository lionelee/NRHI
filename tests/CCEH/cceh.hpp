...
public:
	...
	constexpr static size_type kMask =
		(1 << kSegmentBits) - 1; //...1111 1111
	...
	*/
	...
	public:
		hv_type
		operator()(const key_type &key, size_type sz)
		{
	...
	public:
		bool
		operator()(const key_type &lhs, const key_type &rhs,
			   size_type sz) const
		{
	...
	*/
	struct segment {
		...
		size_type
		segment_insert(const key_type &key, size_type key_len,
			       mapped_type value, size_type value_len,
			       size_t loc, size_t key_hash, pool_base &pop)
		{
			if ((key_hash >>
			...
			for (size_type i = 0;
			     i < kNumPairPerCacheLine * kNumCacheLine; i++) {
				...
				if (slots[slot_idx] != nullptr) {
					hv_type hv = hasher{}(
						slots[slot_idx]->key.get(),
						key_len);
					if ((hv >> (8 * sizeof(key_hash) -
						    local_depth)) == pattern) {
						continue;
					} else {
						transaction::run(pop, [&] {
					...
					transaction::run(pop, [&] {
						...
						slots[slot_idx]->value =
							make_persistent<
								uint8_t[]>(
								value_len);
						slots[slot_idx]->value_len =
							value_len;
						memcpy(slots[slot_idx]
							       ->value.get(),
						       value, value_len);
						slots[slot_idx]->key_len =
							key_len;
						memcpy(slots[slot_idx]->key
								       .get() +
							       8,
		...
		/*
		migrate inserted items to the new segment for segment split
		operation description: For efficient migration, we only copy the
		pointers of inserted items without allocating memory again.
		*/
		void
		Insert4split(persistent_ptr<KV_entry> &old_slot, size_t loc,
			     pool_base &pop)
		{
			for (unsigned i = 0;
			     i < kNumPairPerCacheLine * kNumCacheLine; ++i) {
		...
		persistent_ptr<persistent_ptr<segment>[]>
		Split(pool_base &pop, hv_type key_hash) {
			using namespace std;
			scoped_t lock_segment;
			if (!lock_segment.try_acquire(segment_lock, true))
				return nullptr; // try to acquire segment write
						// lock
			...
			transaction::run(pop, [&] {
				...
				split_segment[1] = make_persistent<segment>(
					local_depth + 1);
			});
			...
			pop.persist((char *)split_segment[1].get(),
				    sizeof(segment));
	...
	struct directory {
		...
		directory(pool_base &pop)
		{
			...
			transaction::run(pop, [&] {
				segments = make_persistent<
					persistent_ptr<segment>[]>(capacity);
			});
		}
	...
	CCEH(size_type _depth)
	{
		...
		transaction::run(pop, [&] {
			...
			for (size_type i = 0; i < pow(2, _depth); i++) {
				dir->segments[i] =
					make_persistent<segment>(_depth);
	...
	ret
	insert(const key_type &key, const mapped_type &value, size_type key_len,
	...
	RETRY:
		...
		size_type rc = target->segment_insert(
			key, key_len, value, value_len, y, key_hash, pop);

		if (rc ==
		    1) // insertion failure: hash collision, need segment split
		{
			persistent_ptr<persistent_ptr<segment>[]> sp_segment =
				target->Split(pop, key_hash);
			...
			sp_segment[1]->pattern =
				((key_hash >> (8 * sizeof(key_hash) -
					       sp_segment[0]->local_depth + 1))
				 << 1) +
				1;

			lock_dir.acquire(
				dir->m,
				true); // acquire write lock for directory

			x = (key_hash >> (8 * sizeof(key_hash) - dir->depth));
			if ((dir->segments[x]->local_depth - 1) <
			    dir->depth) // segment split without directory
					// doubling
			{
				size_type depth_diff =
					dir->depth - sp_segment[0]->local_depth;
				if (depth_diff == 0) {
					if (x % 2 == 0) {
						dir->segments[x + 1] =
							sp_segment[1];
			...
			} else // directory doubling for segment split
			{
				...
				persistent_ptr<persistent_ptr<segment>[]>
					dir_old_segments = dir->segments;
				transaction::run(pop, [&] {
					persistent_ptr<directory> new_dir =
						make_persistent<directory>(
							dir->depth + 1, pop);

					for (size_type i = 0; i < dir->capacity;
					     i++) {
						if (i == x) {
							new_dir->segments[2 *
									  i] =
			...
			goto RETRY;
			   ...
			   2) // insertion failure: the pattern of the item does
			      // not match the pattern of current segment
		{
			goto STARTOVER;
	...
	ret
	get(const key_type &key, size_type key_len)
	{
		hv_type key_hash = hasher{}(key, key_len);
		size_type y = (key_hash & kMask) * kNumPairPerCacheLine;
		/* The directory reader locks protect readers from accessing a
		   reclaimed directory, which guarantees the thread safety for
		   directory. */
		...
		if (target_segment->local_depth < dir->depth) {
			x = (key_hash >> (8 * sizeof(key_hash) -
					  target_segment->local_depth));
		}

		scoped_t lock_target_segment(
			target_segment->segment_lock,
			false); // true：write lock，false: read lock
		for (unsigned i = 0; i < kNumPairPerCacheLine * kNumCacheLine;
		     ++i) {
			size_type location = (y + i) % KNumSlot;
			if (target_segment->slots[location] != nullptr &&
			    target_segment->slots[location]->key.get() != 0x0 &&
			    key_equal{}(
				    key,
				    target_segment->slots[location]->key.get(),
				    key_len)) {
				lock_dir.release();
