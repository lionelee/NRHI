...
#ifdef MACRO_TEST
// the capacity of nrhi for same op num
#define RESERVE_BUCKET_NUM 4194304
#define OPERATION_NUM 64000000
#else
// the capacity of nrhi for same op num
#define RESERVE_BUCKET_NUM 1048576
...
using persistent_map_type =
	nvobj::experimental::concurrent_hash_map<string_t, string_t,
						 string_hasher>;
...
main(int argc, char *argv[])
{
	...
	printf("initial capacity %ld, #inserted %ld\n",
	       pop.root()->cons->bucket_count(), pop.root()->cons->size());
	...
	std::ofstream ofs_throughput("cmap_throughput.res");
...
#ifdef LATENCY_ENABLE
	std::ofstream ofs_latency("cmap_latency.res");
