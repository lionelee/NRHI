...
public:
		...
		cross(level_hash *ht, size_type thread_id)
		{
			...
			if (crossing < count) {
				std::cout << "[Thread-" << thread_id
					  << "] adds crossing to " << crossing
					  << std::endl;
				pthread_cond_wait(&complete, &mutex);
				std::cout << "[Thread-" << thread_id
					  << "] crosses barrier" << std::endl;
			} else {
				std::cout
					<< "[Thread-" << thread_id
					<< "] expands, crossing = " << crossing
					<< std::endl;
				ht->expand();
		...
		clear()
		{
			...
			if (mutexes) {
				for (size_type i = 0; i < capacity; i++) {
					...
					delete_persistent<mutex_t[assoc_num]>(
						mutexes[di]);
				}
				delete_persistent<
					persistent_ptr<mutex_t[assoc_num]>[]>(
					mutexes, capacity);
			}
		}
	};
	    ...
	    : num_threads(n_threads), resize_barrier(n_threads)
	{
		...
		for (size_t i = 0; i < 2; i++) {
			...
			levels[i]->buckets =
				make_persistent<bucket[]>(levels[i]->capacity);
			levels[i]->mutexes = make_persistent<
				persistent_ptr<mutex_t[assoc_num]>[]>(
				levels[i]->capacity);

			for (difference_type idx = 0;
			     idx < (difference_type)levels[i]->capacity; idx++)
				levels[i]->mutexes[idx] =
					make_persistent<mutex_t[assoc_num]>();
		}
	...
	set_thread_num(size_type num)
	{
		...
		std::cout << "switch to " << num_threads << "-thread mode"
			  << std::endl;
	}
		...
		while (true) {
			...
			for (size_t i = 0; i < 2; i++) {
				...
				for (size_t j = 0; j < assoc_num; j++) {
					scoped_t slot_guard(
						levels[i]->mutexes[f_idx][j],
						true);
					if (levels[i]
						    ->buckets[f_idx]
						    .tokens[j] == 0) {
						insert_empty_slot(pop, i, f_idx,
								  j, created);
						return ret(expanded,
							   initial_capacity);
					}
					slot_guard.release();
					slot_guard.acquire(
						levels[i]->mutexes[s_idx][j],
						true);
					if (levels[i]
						    ->buckets[s_idx]
						    .tokens[j] == 0) {
						insert_empty_slot(pop, i, s_idx,
								  j, created);
						return ret(expanded,
							   initial_capacity);
					}
			...
			// 2. Try one-step cuckoo displacements in the same
			// level.
			...
			if (level_expand_time > 0) {
				// f_idx and s_idx were calculated using the
				// capacity of bottom level
	...
	query(const key_type &key, size_type thread_id)
	{
		...
		for (size_type i = 0; i < 2; i++) {
			...
			for (size_type j = 0; j < assoc_num; j++) {
				scoped_t slot_guard(
					levels[i]->mutexes[f_idx][j], false);
			...
			for (size_type j = 0; j < assoc_num; j++) {
				scoped_t slot_guard(
					levels[i]->mutexes[s_idx][j], false);
				...
				if (levels[i]->buckets[f_idx].tokens[j] == 1 &&
				    key_equal{}(
					    levels[i]
						    ...
						    ->first,
					    key)) {
					levels[i]->buckets[f_idx].slots[j] =
						created;
					pop.persist(&levels[i]
							     ->buckets[f_idx]
							     .slots[j]
				...
				if (levels[i]->buckets[s_idx].tokens[j] == 1 &&
				    key_equal{}(
					    levels[i]
						    ...
						    ->first,
					    key)) {
					levels[i]->buckets[s_idx].slots[j] =
						created;
					pop.persist(&levels[i]
							     ->buckets[s_idx]
							     .slots[j]
	...
	expand()
	{
...

		{
			...
			interim_level->buckets = make_persistent<bucket[]>(
				interim_level->capacity);
			interim_level->mutexes = make_persistent<
				persistent_ptr<mutex_t[assoc_num]>[]>(
				interim_level->capacity);

			for (difference_type idx = 0;
			     idx < (difference_type)interim_level->capacity;
			     idx++)
				interim_level->mutexes[idx] =
					make_persistent<mutex_t[assoc_num]>();

			transaction::commit();
		}

		for (difference_type old_idx = 0;
		     old_idx <
		     static_cast<difference_type>(levels[1]->capacity);
		     old_idx++) {
			for (size_type i = 0; i < assoc_num; i++) {
				if (levels[1]->buckets[old_idx].tokens[i] ==
				    1) {
					const key_type &key =
						levels[1]
							->buckets[old_idx]
					...
					if (!succ) {
						std::cerr
							<< "The expanding fails: 3"
							<< std::endl;
						exit(1);
					}

					levels[1]->buckets[old_idx].tokens[i] =
						0;
					pop.persist(&levels[1]
							     ->buckets[old_idx]
							     .tokens[i],
						    sizeof(uint8_t));
				}
	...
	insert_empty_slot(pool_base &pop, size_type level_idx,
			  difference_type bucket_idx, size_type slot_idx,
			  kv_ptr_t e)
	{
		bucket *b;
		if (level_idx < 2) {
		     ...
		     difference_type bucket_idx, kv_ptr_t e)
	{
		for (size_type i = 0; i < assoc_num; i++) {
			scoped_t src_guard(
				levels[level_idx]->mutexes[bucket_idx][i],
				true);
			...
			difference_type f_idx =
				first_index(f_hv, levels[level_idx]->capacity);
			difference_type s_idx =
				second_index(s_hv, levels[level_idx]->capacity);

			difference_type jdx =
				(f_idx == bucket_idx) ? s_idx : f_idx;

			for (size_type j = 0; j < assoc_num; j++) {
				scoped_t dst_guard(
					levels[level_idx]->mutexes[jdx][j],
					true);
				if (levels[level_idx]->buckets[jdx].tokens[j] ==
				    0) {
					insert_empty_slot(
						pop, level_idx, jdx, j,
						levels[level_idx]
					...
					insert_empty_slot(pop, level_idx,
							  bucket_idx, i, e);
	...
	{
		for (size_type i = 0; i < assoc_num; i++) {
			scoped_t src_guard(levels[1]->mutexes[bucket_idx][i],
					   true);
			...
			hv_type s_hv = second_hash(m_key);
			difference_type f_idx =
				first_index(f_hv, levels[0]->capacity);
			difference_type s_idx =
				second_index(s_hv, levels[0]->capacity);

			for (size_type j = 0; j < assoc_num; j++) {
				scoped_t dst_guard(levels[0]->mutexes[f_idx][j],
						   true);
				if (levels[0]->buckets[f_idx].tokens[j] == 0) {
					...
					insert_empty_slot(pop, 1, bucket_idx, i,
							  e);
